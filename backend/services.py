...
async def create_lead(user: _models.User, db: _orm.Session, lead: _schemas.LeadCreate):    
...
async def get_leads(user: _models.User, db: _orm.Session):
    leads = db.query(_models.Lead).filter_by(owner_id=user.id)

    return list(map(_schemas.Lead.from_orm, leads))

async def _lead_selector(lead_id: int, user: _models.User, db: _orm.Session):
...
async def get_lead(lead_id: int, user: _models.User, db: _orm.Session):
    lead = await _lead_selector(lead_id, user, db)

    return _schemas.Lead.from_orm(lead)

async def delete_lead(lead_id: int, user: _models.User, db: _orm.Session):
    lead = await _lead_selector(lead_id, user, db)

    db.delete(lead)
    db.commit()

async def update_lead(lead_id: int, lead: _schemas.LeadCreate, user: _models.User, db: _orm.Session):
